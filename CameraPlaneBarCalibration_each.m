function [camparam,planesol,Optimal_cameraPoints,barPointsCam] = CameraPlaneBarCalibration_each(images_off,images_on,laser,bar,laserthr,barthr)
...
%戻値：1．Cameraパラメータ，2．Laser平面の式@Camera座標系，3．棒上のLaser点群@Camera座標
    ...
    for i = 1:size(images_on.Files,1)
        ...
        CoGPoints = [X,Y];
        %ワールド座標系に変換
        worldPoints = pointsToWorld(camparam.Intrinsics,camparam.RotationMatrices(:,:,i),camparam.TranslationVectors(i,:),CoGPoints);
        newworldPoints = [worldPoints,zeros(size(worldPoints,1),1)];
        %カメラ座標系に変換
        cameraPoints = newworldPoints*camparam.RotationMatrices(:,:,i)+camparam.TranslationVectors(i,:);
